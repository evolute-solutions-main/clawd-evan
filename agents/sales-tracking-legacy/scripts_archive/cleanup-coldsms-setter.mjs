...
for (let i=0;i<data.length;i++){
  ...
  const source = ci(r[3])
  const setter = ci(r[4])
  if (source === 'cold sms' && setter.includes('ads')){
    toFix.push(i+2) // sheet row number
  }
}
const updates = toFix.map(rowNum=>({ range: `'All Booked Calls Data'!E${rowNum}:E${rowNum}`, values: [['']] }))
let updated=0
if (updates.length){ updated = await batchUpdate(updates) }
console.log(JSON.stringify({fixedRows: toFix.length, sample: toFix.slice(0,10), updatedCells: updated}))
